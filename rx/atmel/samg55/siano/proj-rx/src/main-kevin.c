...
void download_cpld_tst()
{
	/////
	// SPI0_CS pin must be assign to PA25
	// ufm_prog_en must have a jump wire from Ateml to CPLD
	//             now I connect to PA11 for testing purpose.

	/// from function: SPI_STATUS Spi0PortRxInit(void)
	pio_set_peripheral(PIOA, PIO_PERIPH_A, PIO_PA9 | PIO_PA10);
	pio_set_peripheral(PIOB, PIO_PERIPH_A, PIO_PB0 );
	spi_master_initialize(0, SPI0_MASTER_BASE, BOARD_FLEXCOM_SPI0);
	spi_set_clock_polarity(SPI0_MASTER_BASE, SPI_CHIP_SEL, 0 /*SPI_CLK_POLARITY*/);
	spi_set_clock_phase(SPI0_MASTER_BASE, SPI_CHIP_SEL, 1 /*SPI_CLK_PHASE*/);
	spi_configure_cs_behavior(SPI0_MASTER_BASE, 0, SPI_CS_KEEP_LOW);
	pio_configure(PIOA, PIO_OUTPUT_1, PIO_PA25, 0);  // manual CS control
	pio_configure(PIOA, PIO_OUTPUT_0, PIO_PA11, 0);  // ufm_prog_en

	delay_ms(50); // Extra waiting for SPI0 Master Ready

	//pio_set(PIOA, PIO_PA11);
pio_configure(PIOA, PIO_OUTPUT_0, PIO_PA0, 0);  // ufm_prog_en
pio_set(PIOA, PIO_PA0);
	pio_clear(PIOA, PIO_PA25);

	uint16_t tmp, tmpw, *pth = &tmp;
	bool spi_tgt_done = true;

	*pth = 0x1; // CMD: read CPLD Version
	spi_tx_transfer(pth, 1, &tmpw, 1, 0);

	spi_rx_transfer(pth, 2, &tmpw, 2, 0); // high 2-byte
	spi_rx_transfer(pth, 2, &tmpw, 2, 0); // Low 2-byte

	pio_set(PIOA, PIO_PA25);

	//pio_clear(PIOA, PIO_PA11);
pio_clear(PIOA, PIO_PA0);
}
