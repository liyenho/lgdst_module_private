...
#ifndef _MAIN_H_
#define _MAIN_H_
#define __REL_DATE__	"June/19/2017"	// update only when released not every build
...
#define TDMA_PERIOD    							/*3600000*/ /*3240000*/ (2*2880000) // see TX ctrl.h for details
#define  TDMA_BOUND								 /*5400000*/ /*4860000*/ (2*4320000) // 100 ms = 12000000
...
typedef struct {
 ...
 #define CONFIG_ADI_6612 // enable adi6612 config
 #ifdef RFFE_PARAMS
 	#define CONFIG_ADI_6612 // forced to configure adi6612 prior to startup
 	#define RF_RX_VAL												0x12
 #endif
 #define SMS_DVBT2_DOWNLOAD // download/init sms device
 #define DIGIBEST_DOWNLOAD  // digibest based download
 ...
 #define RXV4_BOARD         //this shall be the default option in github currently, don't forget..., liyenho
 //#define RXV2_BOARD         //RX board version2 using Max5 80LUT part: Kevin doesn't have REV 2 RX Board
#ifdef CONFIG_ADI_6612
	#ifdef  RXV2_BOARD
	  #define CPLD_6612_TRIG    PIO_PA5    //on schematic: TP43
	#elif defined(RXV4_BOARD)
	  #define CPLD_6612_TRIG    PIO_PA25
	#else
	  #define CPLD_6612_TRIG		PIO_PA15
	#endif
#endif
 //#define I2C_FAKE_POLL
...
#ifdef CONFIG_ADI_6612
  #define RF_RX_FREQ_VAL						0xc
  #define RF_RX_FREQ_VAL_F					0xb
#endif
  #define SMS_BUFFER_SIZE					    (13*4)  //YH: maximum sms TXTXTX msg (remember,WRITE) payload size in bytes
  #define USB_SMS_MSG_IDX						/*0x0*/0x1 // comm intf included
  #define USB_SMS_MSG_LEN						sizeof(sms_access)
/******* SMS4470 related definitions *******/
...
{
	STATE_INITIALIZE =  0,
	STATE_REQ_VERSION_EX,
	STATE_VERSION_EX_RES,
	STATE_FW_DOWNLOAD,
	STATE_REQ_DEVINFO_EX,
	STATE_DEVINFO_EX_RES,
	STATE_REQ_DEV_INIT,
	STATE_DEV_INIT_RES,
	STATE_REQ_POLL_MODE,
	STATE_POLL_MODE_RES,
	STATE_REQ_ENABLE_TS_INTF,
	STATE_ENABLE_TS_INTF_RES,
	STATE_REQ_FW_RELOAD,
	STATE_FW_RELOAD_RES,
	STATE_DATA_DOWNLOAD,
	STATE_REQ_DATA_VALIDITY,
	STATE_DATA_VALIDITY_RES,
	STATE_NORMAL_OPS, // normal operation state
	STATE_REQ_SW_SWITCH_STANDARD,
	STATE_SW_SWITCH_STANDARD_RES,
	STATE_REQ_REMOVE_PID_FILTER,
	STATE_REMOVE_PID_FILTER_RES,
	STATE_REQ_SMS_RF_TUNE,
	STATE_SMS_RF_TUNE_RES,
	STATE_REQ_ADD_PID_FILTER,
	STATE_ADD_PID_FILTER_RES,
	STATE_REQ_DVBT2_OPEN_PLP,
	STATE_DVBT2_OPEN_PLP_RES,
	STATE_REQ_GET_STATISTICS_EX,
	STATE_GET_STATISTICS_EX_RES,
	STATE_GET_I2C_STATUS_IND,
	STATE_WAIT_FOR_IND, // not really being assigned
} Sms4470_State;

// sms4470 device information
...
{
	sms_access	xMsgHeader;
	U32			msgData[1];
} SmsMsgData_ST;  //YH: this is only 12 bytes, for TWI_WRITE,
...
typedef struct SmsDataDownload_S
{
    sms_access		xMsgHeader;
    uint32_t				MemAddr;
    uint32_t				Payload[240/4];
} SmsDataDownload_ST;

typedef struct SMS_MSG_TS_ENABLE
{
    sms_access      xMsgHeader;
    uint32_t            TsClock;                // 0 - TS Clock Speed in Hz
    uint32_t            eTsiMode;               // 1 - TS Mode of operation Serial (on SDIO or HIF Pins), or Parallel
    uint32_t            eTsiSignals;            // 2 - Level of Valid, Sync and Error signals when active
    uint32_t            nTsiPcktDelay;          // 3 - number of delay bytes between TS packets (for 204bytes mode set to 16)
    uint32_t            eTsClockPolarity;       // 4 - Clock edge to sample data
    uint32_t            TsBitOrder;             // 5 - Bit order in TS output
    uint32_t            EnableControlOverTs;    // 6 - Enable Control messages over TS interface
    uint32_t            TsiEncapsulationFormat; // 7 - TS encapsulation method
    uint32_t            TsiPaddingPackets;      // 8 - Number of TS padding packets appended to control messages
    uint32_t            eTsiElectrical;         // 9 - Set slew rate
    uint32_t            IoVoltage;              // 10 - Set IO voltage
	uint32_t            eTsiErrActive;          // 11 - Set ErrActive status
	uint32_t            eTsiClockKeepGo;        // 12 - Set TS clock keep go with no packet or not
} SmsTsEnable_ST;

#define SMS_INIT_MSG_EX(ptr, type, src, dst, len)  \
	(ptr)->msgType = type; (ptr)->msgSrcId = src; (ptr)->msgDstId = dst; \
	(ptr)->msgLength = len; (ptr)->msgFlags = 1/*0*/; // static msg per digibest sdk? liyenho
#define SMS_INIT_MSG(ptr, type, len) \
	SMS_INIT_MSG_EX(ptr, type, 0, HIF_TASK, len)
#define _WAIT0_(ticks)	\
	i2c_read_done = false; \
	systick_enabled = true; \
	g_ul_wait_10ms = ticks;
#define _WAIT_(ticks)	\
	packet_rx.buffer = (uint8_t *) fw_sms_rbuffer; \
	packet_rx.length = sizeof(sms_access)+sizeof(Short_Statistics_ST); \
	_WAIT0_(ticks)
/** sms4470 to host irq assignment */
#define SMS_HOST_INT			PIO_PA26
#define _TWI_READ_ \
	packet_rx.buffer = (uint8_t *) fw_sms_rbuffer; \
	packet_rx.length = sizeof(sms_access)+sizeof(Short_Statistics_ST); \
	i2c_read_done = false; \
...
// Firmware messages processor task IS
	...
	MSG_LAST_MSG_TYPE				= 1000  // Note: Stellar ROM limits this number to 700, other chip sets to 900
...
typedef struct Short_Statistics_S
{
    ...
    /*uint32_t*/int32_t InBandPwr; // modified by liyenho
    uint32_t BER;
    uint32_t SNR;
