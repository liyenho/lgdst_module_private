...
#define tt  CTRL_MAX_SYMBOL_ERR    /* number of errors that can be corrected */
         ...
         for (i=1; i<=nn; i++)
          {  q = 1 ;
             ...
             if (!q)        /* store root and error location number indices */
              { root[count] = i;
                loc[count] = nn-i ; // MSB transmission convention, 1st mesg byte situates highest order of ply term, liyenho
                count++ ;
              };
          } ;
...
void Encode_Control_Packet(uint8_t *source, uint8_t *destination){
  ...
  memcpy(destination, data, CONTROL_MESSAGE_LENGTH);
  memcpy(destination + CONTROL_MESSAGE_LENGTH, bb, PARITY_LENGTH);
...
void Decode_Control_Packet(uint8_t *source, uint8_t *destination)
{
  ...
  for(int i =0;i<PARITY_LENGTH;i++){
    recd[i+nn-PARITY_LENGTH] = source[i+CONTROL_MESSAGE_LENGTH];
  }
