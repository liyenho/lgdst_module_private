...
bool Queue_MavLink_from_USB(uint8_t *pkt_start){
	MavLinkPacket pkt ={0};
	pkt.length = *(pkt_start+1);
	memcpy(&pkt, pkt_start, pkt.length+MAVLINK_HDR_LEN);
	memcpy(&pkt.checksum, pkt_start+pkt.length+MAVLINK_HDR_LEN, MAVLINK_CHKSUM_LEN);
	
	return Queue_MavLink(&outgoing_messages, &pkt);
}
...
#define MSG_TYPE_MAV_TWO_BLOCKS_B	0xDB
	...
	void radio_pkt_filled(uint8_t num_bytes) {
		...
		if (MSG_TYPE_MAV_ONE_BLOCK==msg_type ){
			...
			memcpy(&pkt, pbr1+1, MAVLINK_HDR_LEN+pkt.length);
			//copy checksum information
			memcpy(pkt.checksum, pbr1+MAVLINK_HDR_LEN+pkt.length+1, MAVLINK_CHKSUM_LEN);

			Queue_MavLink(&incoming_messages, &pkt);

			MavLink_Total_Bytes_Used(pkt);
		}else{
			static MavLinkPacket pkt ={0};
				
			if (MSG_TYPE_MAV_TWO_BLOCKS_A == msg_type){
								
				memcpy(&pkt, pbr1+1, num_bytes-1);
				first_half_recv = true;
				bytes_read = num_bytes-1;
				
			}else if (MSG_TYPE_MAV_TWO_BLOCKS_B == msg_type){
				if (first_half_recv){
					memcpy(((uint8_t *)&pkt) + bytes_read, pbr1+1, num_bytes-MAVLINK_CHKSUM_LEN-1);
					memcpy(pkt.checksum, pbr1+1+num_bytes-MAVLINK_CHKSUM_LEN-1, MAVLINK_CHKSUM_LEN);
...
void Control_Send_Event(void){
	#if SEND_MAVLINK
		static uint8_t to_send[300] ={0};
		const block_size = 40;
		//splits MavLink packets into blocks of 40 Bytes
		//padding as necessary to fill out 
		static uint32_t remaining_blocks = 0;
		static uint32_t blocks_used =0;
	
		if (remaining_blocks == 0){
			 MavLinkPacket pkt = {0};
			Get_MavLink(&outgoing_messages, &pkt);
			to_send[0] = MAV_START_SIGN_RDO;
			uint32_t bytes_used = MavLink_PackData(pkt, to_send+1);
			bytes_used+=1; //add 1 for extra start byte added
			
			remaining_blocks = (bytes_used+block_size-1)/block_size;
...
bool Queue_Idle_Mavlink(void){
	...
	*(uint16_t*) idle_pkt.checksum = Compute_Mavlink_Checksum(idle_pkt);
	return Queue_MavLink(&outgoing_messages, &idle_pkt);
}
