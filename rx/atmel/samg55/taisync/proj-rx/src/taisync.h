...
//#define WITH_ANT_SWITCH  // used on ite platform
...
extern volatile uint8_t spi_tgt_done ; // from main.c, liyenho
 #define dev_spi_wr(adr,dly,wr) \
 	...
 	spi_tgt_done = true; wtmp = (wr); \
		*(pth+2) = *(((uint8_t*)&wtmp)+3/*0*/); \
		*(pth+3) = *(((uint8_t*)&wtmp)+2/*1*/); \
		*(pth+4) = *(((uint8_t*)&wtmp)+1/*2*/); \
		*(pth+5) = *(((uint8_t*)&wtmp)+0/*3*/); \
...
#include <assert.h>
 #define dev_spi_rd(adr,dly,rd,dcnt) \
 	...
 	spi_tgt_done = true; \
 	  ...
 	  switch(dcnt) { \
	 	  case 4 : \
			*(((uint8_t*)(rd))+3/*0*/) = *(prh+2) ; \
			*(((uint8_t*)(rd))+2/*1*/) = *(prh+3); \
			*(((uint8_t*)(rd))+1/*2*/) = *(prh+4); \
			*(((uint8_t*)(rd))+0/*3*/) = *(prh+5) ; \
			break ; \
	 	  case 2 : \
			*(((uint8_t*)(rd))+1/*0*/) = *(prh+2) ; \
			*(((uint8_t*)(rd))+0/*1*/) = *(prh+3); \
			break; \
	 	  case 1 : \
			*(((uint8_t*)(rd))+0) = *(prh+2) ; \
			break; \
