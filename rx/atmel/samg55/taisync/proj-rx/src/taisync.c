...
void set_radio_bandwidth(VCH_BW bw) { // enum
	...
	uint8_t dbg_bw;
	//dev_spi_rd(0x03, 1, &dbg_bw, 1) // for debug, liyenho
  	  wtmp = 0x000a | (0xff00 & ((short)0x3<<8));
  	  *(pth+0) = *(((uint8_t*)&wtmp)+0);
  	  *(pth+1) = *(((uint8_t*)&wtmp)+1);
  	  spi_tgt_done = true;
  	  spi_rx_transfer(pth, 6+1, prh, 6+1, 0/*ctrl*/);
  	  while (spi_tgt_done) ; delay_us(/*1*/5);
  	  switch(1) {
	  	  case 4 :
	  	  *(((uint8_t*)(&dbg_bw))+3/*0*/) = *(prh+2) ;
	  	  *(((uint8_t*)(&dbg_bw))+2/*1*/) = *(prh+3);
	  	  *(((uint8_t*)(&dbg_bw))+1/*2*/) = *(prh+4);
	  	  *(((uint8_t*)(&dbg_bw))+0/*3*/) = *(prh+5) ;
	  	  break ;
	  	  case 2 :
	  	  *(((uint8_t*)(&dbg_bw))+1/*0*/) = *(prh+2) ;
	  	  *(((uint8_t*)(&dbg_bw))+0/*1*/) = *(prh+3);
	  	  break;
	  	  case 1 :
	  	  *(((uint8_t*)(&dbg_bw))+0) = *(prh+2) ;
	  	  break;
	  	  default : assert(0);
  	  }
