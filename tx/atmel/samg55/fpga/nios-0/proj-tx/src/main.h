...
void main_remotewakeup_lpm_disable(void);
 ...
 #define dev_spi_wadr(adr,treg,dly) \
 	spi_tgt_done = true; \
 	  *pth = 0xc000 | (0xfff & adr); \
 	  spi_tx_transfer(pth, 2/2, &treg, 2/2, 0/*ctrl/sts*/); \
 	  while (spi_tgt_done) ; delay_us(dly); \

 #define dev_spi_wcnt(acs,treg,dly,wr) \
 	spi_tgt_done = true; \
 	  treg = wr; *pth = acs | (0x0ff & treg); \
 	  spi_tx_transfer(pth, 2/2, &treg, 2/2, 0/*ctrl/sts*/); \
 	  while (spi_tgt_done) ; delay_us(dly); \

 #define dev_spi_wr(adr,treg,dly,wr) \
 	dev_spi_wadr(adr, treg, dly) \
 	dev_spi_wcnt(0xb000, treg, dly, wr)

 #define dev_spi_radr(adr,treg,dly) \
 	spi_tgt_done = true; \
 	  *pth = 0xd000 | (0xfff & adr); \
 	  spi_tx_transfer(pth, 2/2, &treg, 2/2, 0/*ctrl/sts*/); \
 	  while (spi_tgt_done) ; delay_us(dly); \

 #define dev_spi_rcnt(acs,dly,rdp) \
 	spi_tgt_done = true; \
 	  *pth = acs; \
 	  spi_tx_transfer(pth, 2/2, rdp, 2/2, 0/*ctrl/sts*/); \
 	  while (spi_tgt_done) ; delay_us(dly); \

 #define dev_spi_rd(adr,treg,dly,rd) \
 	dev_spi_radr(adr,treg,dly) \
 	dev_spi_rcnt(0x00ff,dly,rd)
