...
void SBIT_W(U8 IO_name, U8 IO_value)
{
	...
	switch (IO_name) {
		case RF4463_PWRDN:
		if (IO_value)
		pio_set(PIOA, (RF4463_TX_MODE)? PIO_PA30:PIO_PA15);
		else
		pio_clear(PIOA, (RF4463_TX_MODE)? PIO_PA30:PIO_PA15);
		break;
		case RF4463_NSEL:
		if (IO_value)
		pio_set(PIOA, (RF4463_TX_MODE)? PIO_PA16:PIO_PA30);
		else
		pio_clear(PIOA, (RF4463_TX_MODE)? PIO_PA16:PIO_PA30);
		break;
		default:
		;
	}
...
U8 SBIT_R(U8 IO_name)
{
	switch (IO_name) {
		case RF4463_PWRDN:
		return pio_get(PIOA, PIO_OUTPUT_1, (RF4463_TX_MODE)? PIO_PA30:PIO_PA15);
		
		case RF4463_NSEL:
		return pio_get(PIOA, PIO_OUTPUT_1, (RF4463_TX_MODE)? PIO_PA16:PIO_PA30);
		
		case RF4463_NIRQ:
		if (RF4463_TX_MODE)
		return pio_get(PIOA, PIO_INPUT, PIO_PA29);
		else
		return pio_get(PIOB, PIO_INPUT, PIO_PB8);
		case RF4463_GPI00:
		if (RF4463_TX_MODE)
		return pio_get(PIOB, PIO_INPUT, PIO_PB3);
		else
		return pio_get(PIOB, PIO_INPUT, PIO_PB13);
		case RF4463_GPI01:
		if (RF4463_TX_MODE)
		return pio_get(PIOB, PIO_INPUT, PIO_PB2);
		else
