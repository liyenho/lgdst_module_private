...
#else
	  // simple random walk (forward-backward) approach
	  /*if (true == fhop_dir) {
	...
	uint32_t shf, wtmp;
		...
		//now keep regs acs on for both tx/rx, liyenho
			...
			while (spi_tgt_done) ; // flush any pending spi xfer
				//spi_tgt_done = true; // not necessary if using acs macro
			switch (pt->access) {
				case READ_CUR:
							pr = gs_uc_hrbuffer1;
  ...
  *(prh-1) = 90+n;
	...
	pr->data[n] = 0xff & (wtmp >> shf);
				...
				default: /* host msg in error */
							continue;
			} //while (spi_tgt_done) ; // not necessary if using acs macro
  		}
#if 0
