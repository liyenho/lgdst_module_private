...
void radio_hal_AssertShutdown(void)
{
  ...
  //SBIT_W(RF4463_PWRDN, 1); //RF_PWRDN = 1
#else
  //PWRDN = 1;
  SBIT_W(RF4463_PWRDN, 1); //PWRDN = 1;
#endif
}
...
{
#if (defined SILABS_PLATFORM_RFSTICK) || (defined SILABS_PLATFORM_LCDBB) || (defined SILABS_PLATFORM_WMB)
  SBIT_W(RF4463_PWRDN, 0); //RF_PWRDN = 0;
#else
  SBIT_W(RF4463_PWRDN, 0); //PWRDN = 0;
#endif
}

void radio_hal_ClearNsel(void)
{
   SBIT_W(RF4463_NSEL, 0); // RF_NSEL = 0;
}

void radio_hal_SetNsel(void)
{
    SBIT_W(RF4463_NSEL, 1); //RF_NSEL = 1;
}

BIT radio_hal_NirqLevel(void)
{
    return SBIT_R(RF4463_NIRQ); //RF_NIRQ;
}
...
void radio_hal_SpiWriteByte(U8 byteToWrite)
{
  ...
  SpiWriteData(1, &byteToWrite); //Kevin: Must use pointer
#endif
}
...
BIT radio_hal_Gpio0Level(void)
{
  ...
  retVal = SBIT_R(RF4463_GPI00); //RF_GPIO0;
#endif
...
BIT radio_hal_Gpio1Level(void)
{
  ...
  retVal = SBIT_R(RF4463_GPI01); //RF_GPIO1;
#endif
#if defined (SILABS_PLATFORM_WMB912)
  #ifdef SILABS_IO_WITH_EXTENDER
    //TODO
