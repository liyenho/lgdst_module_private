...
extern int fhop_base, fhopless/*can be 1, 2 or 3*/, fhop_offset ;
...
U8 bRadio_Check_Tx_RX(void)
{
  if ( SBIT_R(RF4463_NIRQ) == FALSE)//RF_NIRQ == FALSE)
  {
     ctrl_tdma_rxactive=true;
      ...
      {
        /* Packet RX */
        int wrptr_tmp = wrptr_rdo_rpacket;
        /* Get payload length */
        si446x_fifo_info(0x00);
        si446x_read_rx_fifo(Si446xCmd.FIFO_INFO.RX_FIFO_COUNT, &gp_rdo_rpacket_l[intra_packet_wrptr]);
		...
		ctrl_tdma_rxactive=false;
		//reset the pointer in preparation for the next packet
		intra_packet_wrptr = 0;
	  ...
	  if(Si446xCmd.GET_INT_STATUS.PH_PEND & SI446X_CMD_GET_INT_STATUS_REP_PH_PEND_RX_FIFO_ALMOST_FULL_PEND_BIT)
	  {
		...
		si446x_fifo_info(0x00);
        si446x_read_rx_fifo(Si446xCmd.FIFO_INFO.RX_FIFO_COUNT,  &gp_rdo_rpacket_l[intra_packet_wrptr]);
		//increment pointer to new writing offset within the current packet
		//this relies on the FIFO threshold being a multiple of sizeof(uint32_t
		//this condition should be enforced in radio_config.h
		intra_packet_wrptr += (Si446xCmd.FIFO_INFO.RX_FIFO_COUNT/sizeof(uint32_t));
