...
SPI_STATUS Spi0PortRxInit(void)
{
		...
		pio_configure(PIOA, PIO_OUTPUT_1, CPLD_2072_TRIG, 0);  // manual CS control
...
void Page_Erase(uint32_t page) {
	...
	msg.cmd = 0x3;
	msg.rw = 0; // write mode
	msg.o_data = (0xf<<28)|(0xf<<23) | (0x5<<20) | page; // erase pages on CFM0
	Serialize_SPI(&msg);
}
...
/* CPLD image upgrade procedures */
 void download_cpld_fw(cpld_flash_map* fdo, uint32_t *upgrade_fw_hdr )
 {
	...
	if (CPLD_VER != msg.i_data)
		;//printf("cpld version=0x%08x, read back=0x%08x\n",CPLD_VER,msg.i_data);
#ifdef FWM_DNLD_DBG
  	irqflags_t flags;
#endif
	...
	flags = cpu_irq_save();
	  usb_host_active = false;
	cpu_irq_restore(flags);
		usb_write_buf1(fw_dbg_buffer, len1, false);
 #endif
		ul_page_addr += (len1);
	...
	delay_ms(100);
 #ifdef FWM_DNLD_DBG
		usb_write_buf1(fw_dbg_buffer, 160/*flush dbg dump data on host*/, false);
 #endif
