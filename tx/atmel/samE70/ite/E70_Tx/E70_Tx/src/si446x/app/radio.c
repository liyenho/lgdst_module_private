...
COMPILER_ALIGNED(8) tRadioConfiguration RadioConfiguration_915 = RADIO_CONFIGURATION_DATA_915;
COMPILER_ALIGNED(8) tRadioConfiguration RadioConfiguration_869 = RADIO_CONFIGURATION_DATA_869;
tRadioConfiguration *pRadioConfiguration = &RadioConfiguration_915;
...
U8 vRadio_Init(void)
{
  ...
  printf("Power up Si4643\r\n");
  vRadio_PowerUp();

  /* Load radio configuration */
  printf("Using %s configuration\r\n", USE_915MHZ?"915 MHz":"869 MHz");
  if(USE_915MHZ){
  ...
  {
	printf("Configuration_init Failcnt %d\r\n", vradiofailcnt);
    vradiofailcnt++;
	delay_ms(10);
    /* Power Up the radio chip */
    vRadio_PowerUp();
  ...
  printf("vRadio_init finished. Final Failcnt = %u \r\n", vradiofailcnt);
...
{
	/*if (TX_Active){
		printf("Tx_Active lock contention!\r\n");
		TX_Active_lock_cnt++;
...
bool Channel_Scan(void){
	...
	ctrl_tdma_enable = false; //block transmitting while measuring power
...
void Si4463_GetProperties(uint8_t group, uint8_t start_prop, uint8_t num_props){
	
	//wipe memory
	memset(Si4463_Properties, 0x00, sizeof(Si4463_Properties));
	
	printf("Requesting Si4463 Properties \r\nGroup: %#x Start: %#x Num Props %#x\r\n",\
		group, start_prop, num_props);
	si446x_get_property(group, num_props, start_prop);
	memcpy(Si4463_Properties, &Si446xCmd.GET_PROPERTY.DATA, num_props);
	printf("Received Properties: \r\n");
	for(int i =0; i<num_props; i++){
		printf("Property: %#x  Value %#x\r\n", start_prop+i, Si4463_Properties[i]);
	}
	printf("\r\n");
	return;
}